 ...
 */
namespace Magento\WebapiAsync\Model\Async\Swagger;
...
use Magento\BulkApi\Model\Async\Swagger;
use Magento\BulkApi\Model\Async\SwaggerFactory;
use Magento\Webapi\Model\ServiceMetadata;

/**
 * REST Swagger schema generator.
 *
 * Generate ASYNC REST API description in a format of JSON document,
...
class Generator extends AbstractSchemaGenerator
{
    ...
    protected function generateSchema($requestedServiceMetadata, $requestScheme, $requestHost, $endpointUrl)
    {
        ...
        $swagger->setBasePath(strstr($endpointUrl, AsynchronousSchemaRequestProcessor::SCHEMA_PATH, true));
     ...
     * @return array
     */
    private function generateMethodAsyncResponses($exceptionClass, $responses)
    {
        $httpCode = '500';
        $description = 'Internal Server error';
        if (is_subclass_of($exceptionClass, \Magento\Framework\Exception\LocalizedException::class)) {
